 ...
 * * @exports sanitizeAndFormatAiResponse - Main function to clean and format text.
 * @exports cleanAiJsonOutput - Helper to strip code block wrappers from JSON strings.
 */
 ...
 * output: "{\"key\": \"value\"}"
 */
export function cleanAiJsonOutput(text: string): string {
  let clean = text.trim();
 ...
 * 1. **Cleanup:** Strips outer code block wrappers using `cleanAiJsonOutput`.
...
export function sanitizeAndFormatAiResponse(text: string): string {
  ...
  let cleaned = cleanAiJsonOutput(text);
